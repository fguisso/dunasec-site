...
type Supporter = { name: string; logo?: string; url?: string | null; bg?: string | null; invert?: boolean; label?: string; big?: boolean | string }
        ...
        {supportersData.items.length > 0 ? (
          <div className={`${s.lrow} rv`}>
            {supportersData.items.map((item: Supporter, i: number) => {
              ...
              const content = <>
                {item.logo
                  ? <img src={item.logo} alt={item.name} className={`${s.limg} ${item.invert ? s.limgInvert : ''} ${item.big === 'xl' ? s.limgXl : item.big ? s.limgBig : ''}`} loading="lazy" />
                  : item.name
                }
